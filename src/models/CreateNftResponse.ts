...
/**
 * Check if a given object implements the CreateNftResponse interface.
 */
export function instanceOfCreateNftResponse(value: object): boolean {
    let isInstance = true;

    return isInstance;
}

export function CreateNftResponseFromJSON(json: any): CreateNftResponse {
    return CreateNftResponseFromJSONTyped(json, false);
}

export function CreateNftResponseFromJSONTyped(json: any, ignoreDiscriminator: boolean): CreateNftResponse {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'data': !exists(json, 'data') ? undefined : ((json['data'] as Array<any>).map(NftTokenFromJSON)),
        'statusCode': !exists(json, 'status_code') ? undefined : json['status_code'],
    };
}

export function CreateNftResponseToJSON(value?: CreateNftResponse | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
    ...
    return {
        ...
        'data': value.data === undefined ? undefined : ((value.data as Array<any>).map(NftTokenToJSON)),
