...
import type { CreateCollectionRequestMetadataFilesInner } from './CreateCollectionRequestMetadataFilesInner';
import {
    CreateCollectionRequestMetadataFilesInnerFromJSON,
    CreateCollectionRequestMetadataFilesInnerFromJSONTyped,
    CreateCollectionRequestMetadataFilesInnerToJSON,
} from './CreateCollectionRequestMetadataFilesInner';
...
export interface CreateCollectionRequestMetadata {
    ...
    /**
     * Custom attributes or traits for an NFT. You can create 2 types of attributes, fixed or variables. 
     * <br /> **Fixed**: We include these attributes in the metadata of every NFT minted in the collection with a fixed value. You can add attributes like artist name, Twitter, Discord, copyright etc. You define their value here, and then you don't have to submit them during the NFT creation step. 
     * <br /> **Variables**: Here you define a tag, which means that we'll replace this tag with a value you'll have to provide during the NFT creation step. As you can see in the example, we defined five traits (colour, body, eyes, face, and accesory), but we don't specify the value, just a tag between the angle brackets`<>`.
     * @type {string}
     * @memberof CreateCollectionRequestMetadata
     */
    attributes?: string;
...
/**
 * Check if a given object implements the CreateCollectionRequestMetadata interface.
 */
export function instanceOfCreateCollectionRequestMetadata(value: object): boolean {
    let isInstance = true;
...
export function CreateCollectionRequestMetadataFromJSON(json: any): CreateCollectionRequestMetadata {
    return CreateCollectionRequestMetadataFromJSONTyped(json, false);
}

export function CreateCollectionRequestMetadataFromJSONTyped(json: any, ignoreDiscriminator: boolean): CreateCollectionRequestMetadata {
    ...
    return {
        ...
        'files': !exists(json, 'files') ? undefined : ((json['files'] as Array<any>).map(CreateCollectionRequestMetadataFilesInnerFromJSON)),
        'attributes': !exists(json, 'attributes') ? undefined : json['attributes'],
        'version': !exists(json, 'version') ? undefined : json['version'],
    };
}

export function CreateCollectionRequestMetadataToJSON(value?: CreateCollectionRequestMetadata | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
    ...
    return {
        ...
        'files': value.files === undefined ? undefined : ((value.files as Array<any>).map(CreateCollectionRequestMetadataFilesInnerToJSON)),
